...
public class ConstructorRegistration extends javax.swing.JDialog {
    ...
    private void addConCategory() {
        try {
            ...
            ResultSet resultSet = Mysql2.executeSearch("SELECT * FROM `constructor_category`");
    ...
    private void addConAvailability() {
        try {
            ...
            ResultSet resultSet = Mysql2.executeSearch("SELECT * FROM `constructor_availability`");
    ...
    private void AddConstructor() {
        ...
        if (conId.isEmpty()) {
            ...
            try {
                ResultSet resultSet = Mysql2.executeSearch("SELECT * FROM `constructor` WHERE `mobile`='" + conMobile1 + "' OR `email`='" + conEmail + "'");

                if (resultSet.next()) {
                    JOptionPane.showMessageDialog(this, "This constructor already registerd.", "Warning", JOptionPane.WARNING_MESSAGE);
                } else {
                    Mysql2.executeIUD("INSERT INTO `constructor`(`id`,`first_name`,`last_name`,`mobile`,`email`,`charge`,`date`,"
    ...
    private void updateConstructor() {
        ...
        if (conId.isEmpty()) {
            ...
            try {
                ...
                if (canUpdate) {
                    Mysql2.executeIUD("UPDATE `constructor` SET `first_name`='" + confFname + "',`last_name`='" + conLname + "',`mobile`='" + conMobile1 + "',`email`='" + conEmail + "',"
