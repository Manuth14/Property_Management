...
public class ConstructorCompanyManage extends javax.swing.JPanel {
    ...
    private void loadConComapany(String column, String orderby, String comName) {
        ...
        try {
            ResultSet resultSet = Mysql2.executeSearch("SELECT * FROM `company` INNER JOIN `address` ON"
                    + " `company`.`address_id`=`address`.`id` INNER JOIN `city` ON"
                    + " `address`.`city_id`=`city`.`id` WHERE `company_name` LIKE '%" + comName + "%' ORDER BY " + column + " " + orderby + "");
            ...
            while (resultSet.next()) {
                ...
                vector.add(resultSet.getString("address.line_01"));
                vector.add(resultSet.getString("address.line_02"));
                vector.add(resultSet.getString("address.id"));
                model.addRow(vector);
            }
    ...
    private void initComponents() {
        ...
        jLabel12.setIcon(new javax.swing.ImageIcon(getClass().getResource("/resources/back2.png"))); // NOI18N
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (conComId.equals("ComID")) {
            ...
            try {
                ResultSet resultSet1 = Mysql2.executeSearch("SELECT * FROM `company` WHERE `company_name`='" + conComName + "' OR `hotline` = '" + conComHotline1 + "' OR `email`='" + conComEmail + "'");
                if (resultSet1.next()) {
                    JOptionPane.showMessageDialog(this, "This company Name or Hotline or Email alreaday used");
                } else {

                    Mysql2.executeIUD("INSERT INTO `address`(`line_01`,`line_02`,`city_id`) "
                            + "VALUES('" + conComLine1 + "','" + conComLine2 + "','" + cityMap.get(conComCity) + "')");

                    ResultSet resultSet2 = Mysql2.executeSearch("SELECT * FROM `address` WHERE `line_01`='" + conComLine1 + "' AND "
                    ...
                    Mysql2.executeIUD("INSERT INTO `company`(`id`,`company_name`,`hotline`,`email`,`address_id`) "
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        if (!addCityField.getText().isEmpty()) {
            ...
            try {
                ResultSet resultSet = Mysql2.executeSearch("SELECT * FROM `city` WHERE `city_name`='" + addCiry + "' ");
                ...
                if (resultSet.next()) {
                    ...
                    Mysql2.executeIUD("INSERT INTO `city`(`city_name`) VALUES('" + addCiry + "')");
                    loadCities();
                    addCityField.setText("");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        if (row == -1) {
            ...
            if (conComId.equals("ComID")) {
                ...
                try {
                    ResultSet resultSet = Mysql2.executeSearch("SELECT * FROM `company` WHERE (`company_name`='" + conComName + "' OR `email`='" + conComEmail + "' OR `hotline`='" + conComHotline1 + "') AND `id` != '" + conComId + "'");

                    if (resultSet.next()) {
                        JOptionPane.showMessageDialog(this, "This Company name or Email Already Registered", conComId, HEIGHT);
                    } else {
                        Mysql2.executeIUD("UPDATE `address` SET `line_01`='" + conComLine1 + "',`line_02`='" + conComLine2 + "',`city_id`='" + cityMap.get(conComCity) + "' WHERE `id`='" + addressId + "'");

                        ResultSet resultSet1 = Mysql2.executeSearch("SELECT * FROM `address` WHERE `line_01`='" + conComLine1 + "' AND "
                                + "`line_02`='" + conComLine2 + "' AND "
                                + "`city_id`='" + cityMap.get(conComCity) + "'");
                        ...
                        Mysql2.executeIUD("UPDATE `company` SET `company_name`='" + conComName + "',`hotline`='" + conComHotline1 + "',`email`='" + conComEmail + "',"
